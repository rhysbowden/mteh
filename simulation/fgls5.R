...
fgls5=function(Tlist,Clist,ni){
    ...
    for(T_ind in 1:length(Tlist)){
        for(C_ind in 1:length(Clist)){
            ...
            olsmb = solve(t(Xb)%*%Xb)%*%t(Xb)# % matrix for getting ordinary least squares estimates of fixed effects, olsm*y
            ...
            for(ZZind in 1:numZ){
                for(PPind in 1:numP){
                    Kbase[ZZind,PPind] = sum(diag(t(ZZ1[[ZZind]])%*%PP[[PPind]]%*%ZZ2[[ZZind]]))
                }
            }
